...
export class UserDao {
  ...
  // 根据一个id数组判断一组用户是否都存在，返回不存在的用户的id的数组
  async isUsersExist(
    userIdList: number[],
    conn?: mysql.Connection,
  ): Promise<number[]> {
    const sql = `select id from users where id in (${userIdList.map(() => '?').join(',')}) and is_deleted = 0`;
    const res = await this.dbService.query<User>(sql, [...userIdList], conn);
    if (res.length === userIdList.length) return [];
    return res
      .map((user) => (userIdList.includes(user.id) ? null : user.id))
